...
This package includes two example datasets (probe level): uniformly-handled data `r.data.pl` and non-uniformly-handled data `non.r.data.pl`, both coming from the 5% random subset of the original data. Here is a glimpse of the datasets. The last character of the sample labels "E" or "V" indicates whether the sample is an endometrial or ovarian tumor. To call the data, simply use the commands below:
...
2. scaling 1 - per selected array, expanding the variance of probes towards the array's inter-quartiles; probes that are outside of the inter-quartile range remain unchanged.
